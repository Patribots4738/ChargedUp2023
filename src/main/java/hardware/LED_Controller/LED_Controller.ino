...
#define BELLYPAN_END_INDEX 45
#define SPONSOR_PANEL_START_INDEX 10
#define SPONSOR_PANEL_END_INDEX 14
#define ARM_START_INDEX 20
#define ARM_END_INDEX 24
...
void rainbow(int startIndex, int endIndex, int cycles, int speed){ // TODO directionk
    if (rainbowIncrementer < 256){
      fill_rainbow( leds, NUM_LEDS, rainbowIncrementer, 7);
      FastLED.show();
      delay(speed);
...
void setBellyPan(int pattern){
  switch(pattern){
    ...
    case 5: // Blue
      flash(BELLYPAN_START_INDEX, BELLYPAN_END_INDEX, CRGB::Blue);
      break;
