...
public class ADIS16470_IMU implements AutoCloseable, NTSendable {
     ...
     * while the
     * calibration is in progress, this is typically done when the robot is first
     * turned on while it's
     ...
     * @return 1 if the new yaw axis is the same as the current one, 2 if the switch
     *         to Standard SPI
     *         failed, else 0.
     */
        ...
        while (true) {
            ...
            if (m_thread_active) {
                ...
                for (int i = 0; i < data_to_read; i += dataset_len) {
                     ...
                     * (based on timestamp)
                     */
                    delta_angle = (toInt(buffer[i + 3], buffer[i + 4], buffer[i + 5], buffer[i + 6]) * delta_angle_sf)
